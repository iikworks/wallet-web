      ...
      <nav className={`flex flex-col justify-between bg-white ${opened ? 'w-72' : 'w-16 sm:w-24'} h-max min-h-screen transition-all`}>
        <div className="space-y-3 px-2 v sm:px-5 py-4">
          {store.user &&<div className={`flex gap-3 ${opened ? 'items-center' : 'justify-center'} overflow-hidden`}>
            ...
            {opened &&<div>
              <div className="font-medium text-base whitespace-nowrap">{`${store.user.first_name} ${store.user.last_name}`}</div>
            ...
            {menu.map(element => {
              return (
                <Link key={element.link} to={element.link}
                      className={`flex items-center ${opened ? '' : 'justify-center'} whitespace-nowrap h-10 font-medium gap-2 w-full rounded-md px-4 hover:bg-gray-200 border border-gray-200 transition`}>
                  {element.icon}
